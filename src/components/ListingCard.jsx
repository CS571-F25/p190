...
//     setSaved(now)
//     toast(now ? 'Saved to Shortlist' : 'Removed from Shortlist', now ? 'success' : 'secondary', saveBtnRef.current)
//   }

//   return (
//     <Card className="h-100 shadow-sm">
//       <div className="ratio ratio-16x9 bg-light rounded-top">
//         <MapImage lat={item.latitude} lon={item.longitude} alt={`Map of ${item.formattedAddress}`} className="object-fit-cover rounded-top w-100 h-100" />
//       </div>
...
//           <Button ref={saveBtnRef} onClick={toggleSave} variant={saved ? 'success' : 'outline-success'} size="sm">{saved ? 'Saved' : 'Save'}</Button>
//           <OpenInMapsButton lat={item.latitude} lon={item.longitude} address={item.formattedAddress} size="sm" />
...
export default function ListingCard({ item, compare, onToggleCompare }) {
  ...
  const compareId = `compare-${item.id}`
  ...
  return (
    <Card className="h-100 shadow-sm">
      ...
      <Card.Body className="d-flex flex-column">
        ...
        <div className="mt-auto d-flex gap-2 align-items-center flex-wrap">
          <Button as={Link} to={`/listing/${encodeURIComponent(item.id)}`} state={{ item }} variant="outline-primary" size="sm">
            Details
          </Button>
          <Button ref={saveBtnRef} onClick={toggleSave} variant={saved ? 'success' : 'outline-success'} size="sm">
            {saved ? 'Saved' : 'Save'}
          </Button>
          <OpenInMapsButton lat={item.latitude} lon={item.longitude} address={item.formattedAddress} size="sm" />
